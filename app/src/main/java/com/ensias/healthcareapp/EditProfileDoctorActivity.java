...
public class EditProfileDoctorActivity extends AppCompatActivity {
    ...
    private static final String TAG = "EditProfileDoctorAct";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        updateProfile.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // Then upload image (will return to previous activity when complete)
                uploadProfileImage();
            }
        });
    }
        ...
        documentReference.update("tel", phone)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        Toast.makeText(EditProfileDoctorActivity.this, "Info Updated", Toast.LENGTH_SHORT).show();
                        // Only finish the activity here if image upload is not happening
                        // or coordinate with image upload to ensure both complete before returning
                        if (uriImage == null) {
                            finish();
                        }
                    }
    ...
    private void uploadProfileImage() {
        if (uriImage != null) {
            ...
            AlertDialog dialog = new SpotsDialog.Builder().setContext(this).setCancelable(false).build();
            dialog.show();

            // Create a clear reference - note we're using the email directly as filename
            final StorageReference fileReference = FirebaseStorage.getInstance()
                    .getReference("DoctorProfile")
                    .child(currentDoctorUID + ".jpg");
            ...
            fileReference.putFile(uriImage)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            // Get the download URL specifically for the file we just uploaded
                            fileReference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                @Override
                                public void onSuccess(Uri downloadUri) {
                                    // Log the success and URL for debugging
                                    Log.d(TAG, "Image uploaded successfully. URL: " + downloadUri.toString());

                                    // Update database with image URL - choose ONE approach:
                                    // Option 1: Using Realtime Database
                                    UploadImage upload = new UploadImage(currentDoctorUID, downloadUri.toString());
                                    pDatabaseRef.child(currentDoctorUID).setValue(upload);

                                    // Option 2: Using Firestore (if you prefer)
                                    // doctorRef.collection("Doctor").document(doctorID)
                                    //     .update("profileImageUrl", downloadUri.toString());

                                    dialog.dismiss();
                                    Toast.makeText(EditProfileDoctorActivity.this,
            ...
            // If no image was selected, just update doctor info and return
            Toast.makeText(this, "No image selected", Toast.LENGTH_SHORT).show();
            updateDoctorInfos(doctorName.getText().toString(),
